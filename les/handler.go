...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light PFCoin peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light PFCoin handshake failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light PFCoin message handling failed", "err", err)
			return err
		}
	}
}
